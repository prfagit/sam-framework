...
class MemoryManager:
    ...
    async def save_session(self, session_id: str, messages: List[Dict]):
        ...
        async with aiosqlite.connect(self.db_path) as conn:
            ...
            cursor = await conn.execute('SELECT created_at FROM sessions WHERE session_id = ?', (session_id,))
    ...
    async def get_user_preference(self, user_id: str, key: str) -> Optional[str]:
        ...
        logger.debug(f"Retrieved preference {key} for user {user_id}: {'found' if value else 'not found'}")
        return value
    ...
    async def get_secure_data(self, user_id: str) -> Optional[Dict[str, str]]:
        ...
        logger.debug(f"Retrieved secure data for user {user_id}: {'found' if data else 'not found'}")
        return data
