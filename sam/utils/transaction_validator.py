...
class TransactionValidator:
    ...
    async def validate_balance_sufficient(
        ...
        if transaction_amount > self.large_trade_threshold:
            try:
                ...
                warnings.append(
                    f"Large transaction: {transaction_amount:.4f} SOL (~${usd_value:.2f})"
                )
    ...
    async def validate_slippage_settings(
        ...
        if slippage < 1 or slippage > 50:
            ...
            return ValidationResult(
                is_valid=False, warnings=warnings, errors=errors, suggestions=suggestions
            )
        ...
        if slippage > self.high_slippage_threshold:
            ...
            if slippage > 20:
                ...
                suggestions.append(
                    "Consider lowering slippage or waiting for better market conditions"
                )
            else:
        ...
        return ValidationResult(
            is_valid=True, warnings=warnings, errors=errors, suggestions=suggestions
        )
    ...
    async def validate_pump_fun_sell(
        ...
        return ValidationResult(
            is_valid=True, warnings=all_warnings, errors=all_errors, suggestions=all_suggestions
        )
...
async def validate_pump_buy(
    wallet_balance: float, amount: float, slippage: int, mint: str
) -> ValidationResult:
...
async def validate_sol_transfer(
    wallet_balance: float, amount: float, to_address: str
) -> ValidationResult:
