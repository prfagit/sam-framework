...
class TestWebSession:
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_get_agent_singleton(self, mock_builder_class):
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_get_agent_exception_handling(self, mock_builder_class):
        ...
        mock_builder = MagicMock()
        mock_builder.build = AsyncMock(side_effect=Exception("Build failed"))
        mock_builder_class.return_value = mock_builder
    ...
    @patch("sam.web.session.AgentBuilder")
    def test_run_once(self, mock_builder_class):
        ...
        with patch("sam.web.session.get_agent", side_effect=mock_get_agent):
            ...
            mock_agent.run.assert_called_once_with("test prompt", "test_session")

    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_basic(self, mock_builder_class):
        ...
        with patch("sam.web.session.get_event_bus") as mock_get_bus:
            mock_bus = MagicMock()
            ...
            async with run_with_events("test prompt", "test_session") as event_iter:
                ...
                mock_agent.run.assert_called_once_with(
                    "test prompt", "test_session", publish_final_event=False
                )

    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_with_real_events(self, mock_builder_class):
        ...
        async with run_with_events("test prompt", "test_session") as event_iter:
            ...
            bus = get_event_bus()
            await bus.publish("agent.delta", {"session_id": "test_session", "content": "test delta"})
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_session_filtering(self, mock_builder_class):
        ...
        async with run_with_events("test prompt", "test_session") as event_iter:
            ...
            await bus.publish("agent.delta", {"session_id": "test_session", "content": "our session delta"})
            # Event for different session - should be filtered out
            await bus.publish("agent.delta", {"session_id": "other_session", "content": "other session delta"})
            ...
            assert any(e["event"] == "agent.delta" and e["payload"]["content"] == "our session delta" for e in events)

    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_cleanup(self, mock_builder_class):
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_agent_run_failure(self, mock_builder_class):
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_delta_streaming(self, mock_builder_class):
    ...
    @pytest.mark.asyncio
    @patch("sam.web.session.AgentBuilder")
    async def test_run_with_events_empty_response(self, mock_builder_class):
